...
public isolated class AccountService {
    ...
    public isolated function getAccountBalances(string accountId) returns model:BalanceResponse|model:InvalidResourceIdError {
        if accountId == "" {
            log:printDebug(util:EMPTY_ACCOUNT_ID);
            return error(util:EMPTY_ACCOUNT_ID, ErrorCode=util:CODE_EMPTY_ACCOUNT_ID);
        }
    ...
    # + accountId - the account ID path parameter, Try A001 as the default value
    # + return - account beneficiaries list or error
    public isolated function getAccountBeneficiaries(string accountId) returns model:BeneficiariesResponse|model:InvalidResourceIdError {
        if accountId == "" {
            log:printDebug(util:EMPTY_ACCOUNT_ID);
            return error(util:EMPTY_ACCOUNT_ID, ErrorCode=util:CODE_EMPTY_ACCOUNT_ID);
        }
    ...
    # + accountId - the account ID path parameter, Try A001 as the default value
    # + return - account direct debits list or error
    public isolated function getAccountDirectDebits(string accountId) returns model:DirectDebitsResponse|model:InvalidResourceIdError {
        if accountId == "" {
            log:printDebug(util:EMPTY_ACCOUNT_ID);
            return error(util:EMPTY_ACCOUNT_ID, ErrorCode=util:CODE_EMPTY_ACCOUNT_ID);
        }
    ...
    # + accountId - the account ID path parameter, Try A001 as the default value
    # + return - account offers list or error
    public isolated function getAccountOffers(string accountId) returns model:OffersResponse|model:InvalidResourceIdError {
        if accountId == "" {
            log:printDebug(util:EMPTY_ACCOUNT_ID);
            return error(util:EMPTY_ACCOUNT_ID, ErrorCode=util:CODE_EMPTY_ACCOUNT_ID);
        }
    ...
    # + accountId - the account ID path parameter, Try A001 as the default value
    # + return - account parties list or error
    public isolated function getAccountParties(string accountId) returns model:PartiesResponse|model:InvalidResourceIdError {
        if accountId == "" {
            log:printDebug(util:EMPTY_ACCOUNT_ID);
            return error(util:EMPTY_ACCOUNT_ID, ErrorCode=util:CODE_EMPTY_ACCOUNT_ID);
        }
    ...
    # + accountId - the account ID path parameter, Try A001 as the default value
    # + return - account parties list or error
    public isolated function getAccountParty(string accountId) returns model:PartiesResponse|model:InvalidResourceIdError {
        if accountId == "" {
            log:printDebug(util:EMPTY_ACCOUNT_ID);
            return error(util:EMPTY_ACCOUNT_ID, ErrorCode=util:CODE_EMPTY_ACCOUNT_ID);
        }
    ...
    # + accountId - the account ID path parameter, Try A001 as the default value
    # + return - account products list or error
    public isolated function getAccountProducts(string accountId) returns model:ProductsResponse|model:InvalidResourceIdError {
        if accountId == "" {
            log:printDebug(util:EMPTY_ACCOUNT_ID);
            return error(util:EMPTY_ACCOUNT_ID, ErrorCode=util:CODE_EMPTY_ACCOUNT_ID);
        }
    ...
    # + accountId - the account ID path parameter, Try A001 as the default value
    # + return - account scheduled payments list or error
    public isolated function getAccountScheduledPayments(string accountId) returns model:ScheduledPaymentsResponse|model:InvalidResourceIdError {
        if accountId == "" {
            log:printDebug(util:EMPTY_ACCOUNT_ID);
            return error(util:EMPTY_ACCOUNT_ID, ErrorCode=util:CODE_EMPTY_ACCOUNT_ID);
        }
    ...
    # + accountId - the account ID path parameter, Try A001 as the default value
    # + return - account standing orders list or error
    public isolated function getAccountStandingOrders(string accountId) returns model:StandingOrdersResponse|model:InvalidResourceIdError {
        if accountId == "" {
            log:printDebug(util:EMPTY_ACCOUNT_ID);
            return error(util:EMPTY_ACCOUNT_ID, ErrorCode=util:CODE_EMPTY_ACCOUNT_ID);
        }
    ...
    # + accountId - the account ID path parameter, Try A001 as the default value
    # + return - account statements list or error
    public isolated function getAccountStatements(string accountId) returns model:StatementsResponse|model:InvalidResourceIdError {
        if accountId == "" {
            log:printDebug(util:EMPTY_ACCOUNT_ID);
            return error(util:EMPTY_ACCOUNT_ID, ErrorCode=util:CODE_EMPTY_ACCOUNT_ID);
        }
    ...
    # + return - account statements list or error
    public isolated function getAccountStatement(string accountId, string statementId) returns model:StatementsResponse|model:InvalidResourceIdError {
        if accountId == "" {
            log:printDebug(util:EMPTY_ACCOUNT_ID);
            return error(util:EMPTY_ACCOUNT_ID, ErrorCode=util:CODE_EMPTY_ACCOUNT_ID);
        } else if statementId == "" {
    ...
    # + statementId - the statement ID path parameter, Try S001 as the default value
    # + return - account statements list or error
    public isolated function getAccountStatementFile(string accountId, string statementId) returns http:Response|model:InvalidResourceIdError {
        if accountId == "" {
            log:printDebug(util:EMPTY_ACCOUNT_ID);
            return error(util:EMPTY_ACCOUNT_ID, ErrorCode=util:CODE_EMPTY_ACCOUNT_ID);
        } else if statementId == "" {
    ...
    # + accountId - the account ID path parameter, Try A001 as the default value
    # + return - account transactions list or error
    public isolated function getAccountTransactions(string accountId) returns model:TransactionsResponse|model:InvalidResourceIdError {
        if accountId == "" {
            log:printDebug(util:EMPTY_ACCOUNT_ID);
            return error(util:EMPTY_ACCOUNT_ID, ErrorCode=util:CODE_EMPTY_ACCOUNT_ID);
        }
